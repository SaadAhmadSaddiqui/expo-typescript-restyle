...
import { InputRefType, CommonAnimatedPropsTypes, AnimatedViewPropsType, AnimatedTextPropsType } from "./types";
import { getHelpText } from "./utils";
import { IconNameType } from "../Icon/types";

export const useInputRef: InputRefType = () => useRef(null);

export const useInputValue = ({ value, defaultValue }: { value?: string; defaultValue?: string }) => {
...
export const useTogglePasswordVisibility = (secureTextEntry: boolean) => {
	...
	const [passwordVisibilityIcon, setPasswordVisibilityIcon] = useState<IconNameType>("eye-on");
...
export const useOutlineLabelVisibility = ({
	...
	const duration = 200;
	const commonAnimatedProps: CommonAnimatedPropsTypes = useMemo(
		() => ({
			duration,
	...
	const labelLineHeightValue: number = theme.textVariants.subtitle2Regular.fontSize;
	const initialTopValue: number = (inputHeight - labelLineHeightValue + 2) / 2;
	const labelPositionEmptyValue = 0;
	const labelPositionFillValue: number = labelLineHeightValue / 2 + initialTopValue - 2;

	const labelPositionRef = useRef(new Animated.Value(value ? labelPositionFillValue : labelPositionEmptyValue)).current;
	const fontSizeRef = useRef(new Animated.Value(value ? theme.textVariants.subtitle4Regular.fontSize : theme.textVariants.defaults.fontSize)).current;
	const lineHeightRef = useRef(
		new Animated.Value(value ? theme.textVariants.defaults.fontSize : theme.textVariants.subtitle2Regular.fontSize),
	).current;
	...
	}, [commonAnimatedProps, fontSizeRef, labelFontSize, labelLineHeightValue, labelPositionFillValue, labelPositionRef, lineHeightRef]);
	...
	}, [commonAnimatedProps, fontSizeRef, labelFontSize, labelLineHeightValue, labelPositionRef, lineHeightRef]);
